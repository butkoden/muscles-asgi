...
from muscles_asgi import AsgiStrategy
...
- если `logger` не передан, используется логгер `muscles.asgi`.
...
- исправлена обработка `async` handler (awaitable результат корректно ожидается).
